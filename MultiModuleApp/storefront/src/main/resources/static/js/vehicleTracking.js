	...
	else if (event.id === 'GetVehicleByName') 
	{
        ...
        "<body>" +
        			//First Div starts here
         			...
         			"<div class='jsCss_forDivs' style='height:390px;'>" +				                    					                   
         				"<input type='text' name='carName' placeholder='Enter vehicle name' id='js-vehicle-search-vehicleName' autofocus='autofocus' value=''  required='true' maxlength='255' class='forInputTypeTextAndSelect'><br/><br/>" +
...
function ajax_showAllVehicles(){
	...
	$.ajax({
		url:"http://localhost:8888/VehicleTrackingSystem/dispatcher/vehicle",
		type:"GET",
...
function ajax_findVehicle_By_Name(){
    ...
    else
    {
		...
		$.ajax({
			url:"http://localhost:8888/VehicleTrackingSystem/dispatcher/vehicle/name/"+ vehicleName.toUpperCase(),
			type:"GET",
			data:{profileGuid: "", userName: ""},
			success: function(data){
...
function ajax_AllProfiles(){
	//$('.js-unregister-account').after("<img class='loading-img js-loading-img' alt='Loading' src='static/asset/my_country_spinner.gif'>");
	$.ajax({
		url:"http://localhost:8888/VehicleTrackingSystem/dispatcher/profile",
		type:"GET",
		data:{profileGuid: "", userName: ""},
		success: function(data){
			var htmlValue = "<html><header><title>Yoogesh Profile</title></header>" +
...
function ajax_saveVehicle(){
	//$('.js-unregister-account').after("<img class='loading-img js-loading-img' alt='Loading' src='static/asset/my_country_spinner.gif'>");
	$.ajax({
		url:"http://localhost:8888/VehicleTrackingSystem/dispatcher/save-vehicle-using-ajax",
		type:"GET",
		data:{profileGuid: "", userName: ""},
		success: function(data){
